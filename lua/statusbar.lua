...
diagnostics_message.default = {
  colors = {
    error = utils.extract_color_from_hllist({"fg", "sp"}, {
      "DiagnosticError",
      "LspDiagnosticsDefaultError",
      "DiffDelete"
    }, "#e32636"),
    warning = utils.extract_color_from_hllist({"fg", "sp"}, {
      "DiagnosticWarn",
    ...
    hint = utils.extract_color_from_hllist({"fg", "sp"}, {
      "DiagnosticHint",
...
function diagnostics_message:init(options)
  ...
  }, "diagnostics_message_error", self.options)
  self.highlights.warn = highlight.create_component_highlight_group({fg = self.options.colors.warn},
                                                                    "diagnostics_message_warn",
                                                                    self.options)
  self.highlights.info = highlight.create_component_highlight_group({fg = self.options.colors.info},
                                                                    "diagnostics_message_info",
                                                                    self.options)
  self.highlights.hint = highlight.create_component_highlight_group({fg = self.options.colors.hint},
...
function diagnostics_message:update_status(is_focused)
  ...
  if #diagnostics > 0 then
    ...
    return highlight.component_format_highlight(hl[top.severity]) .. icons[top.severity] .. " " ..
...
require('lualine').setup({
  ...
  winbar = {},
  inactive_winbar = {},
  extensions = {'nvim-tree', 'mason', 'nvim-dap-ui'}
})
